...
### Rules already tested in other sections
- **OBS_X_TO_OBSERVABLE_SINK** — Already tested in test_1_2 (1_2_OBS_X_TO_OBSERVABLE_SINK-x_bits_to_observable).
...
### Happy-path and edge-case scenarios (not testable in lint framework)
- All scenarios in sections 3.1-3.2 (happy path, boundary/edge cases) verify runtime expression evaluation (two's complement, shift fill, carry capture), which are behavioral/simulation-level checks, not lint diagnostics.
...
### Happy-path and edge-case scenarios (not testable in lint framework)
- Default Rising edge, Falling edge, Immediate/Clocked reset types, reset active high/low, multiple SYNC blocks with different clocks, empty SYNC body, register read/write — these verify correct parsing and code generation, not lint diagnostics.
- No-reset SYNC blocks, register read in ASYNC — structural parsing/behavioral scenarios without diagnostics.
...
### Happy-path and edge-case scenarios (not testable in lint framework)
- Valid BIT/BUS/FIFO/HANDSHAKE/PULSE/MCP/RAW crossings, bidirectional CDC, default n_stages=2, source in correct SYNC domain, dest alias in correct SYNC domain, n_stages=1/8, source in ASYNC — these verify correct parsing and CDC bridge instantiation, not lint diagnostics.
...
### Happy-path and edge-case scenarios (not testable in lint framework)
- Valid @feature with @else (both branches semantically valid), feature in REGISTER/ASYNCHRONOUS/SYNCHRONOUS blocks, CONFIG and CONST expression evaluation, logical operators in conditions, comparison operators, always-true/always-false conditions, empty feature body, feature around entire module body — these verify correct parsing and code generation, not lint diagnostics.
