...
subtitle: "State: Beta — Version: 0.1.4"
toc: true
header-includes:
  - \usepackage{titling}
...
<PIN_BLOCK> {
  <pin_name> = {
    ...
    reset=<signal_name>                     // required when mode=DIFFERENTIAL on OUT
  };
}
```
...
6. **Serialization Attributes:** `fclk`, `pclk`, and `reset` are all required when `mode=DIFFERENTIAL` on output pins. `fclk` and `pclk` must reference valid clock names declared in `CLOCKS` or generated by `CLOCK_GEN`. `reset` must reference a valid signal name, typically a `CLOCK_GEN` `LOCK` output.
